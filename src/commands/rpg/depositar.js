...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('depositar')
    .setDescription('🏦 Depositar lupins da carteira para o banco')
    .addIntegerOption(opt => opt.setName('quantidade').setDescription('Quantidade para depositar').setMinValue(1).setRequired(true)),
  category: 'rpg',
  cooldown: 3,
  permissions: [],
  async execute(interaction) {
    try {
      ...
      logger.info(`${interaction.user.tag} executou /depositar ${amount}`);
      ...
      if (amount <= 0) {
        const embed = new EmbedBuilder()
          ...
          .setDescription('Informe uma quantidade positiva para depositar.')
          .setTimestamp();
        return interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
      }
      if (wallet < amount) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(`Você tem **${wallet}** ${LUPINS_EMOJI} na carteira, mas quer depositar **${amount}**.`)
          .setTimestamp();
        return interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral });
      }
      ...
      const embed = new EmbedBuilder()
        .setColor(ORANGE)
        .setTitle('Confirmar Depósito')
        .setDescription(`Deseja depositar **${amount}** ${LUPINS_EMOJI} no banco?`)
        .addFields(
      ...
      const confirmBtn = new ButtonBuilder()
        .setCustomId(`econ_deposit_confirm_${userId}_${amount}`)
        .setLabel('✅ Confirmar')
        .setStyle(ButtonStyle.Success);
      const cancelBtn = new ButtonBuilder()
        .setCustomId(`econ_deposit_cancel_${userId}`)
        .setLabel('❌ Cancelar')
